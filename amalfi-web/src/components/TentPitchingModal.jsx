...
import { addDaysToDateOnly, diffDateOnlyDays, formatDateOnlyInManila, formatNowInManila, getManilaTodayKey } from '../utils/manilaDate';
...
const diffNights = (ci, co) => { if (!ci || !co) return 0; return Math.max(0, diffDateOnlyDays(ci, co)); };
...
const TentPitchingModal = ({ onClose }) => {
    ...
    const minOut      = checkIn ? addDaysToDateOnly(checkIn, 1) : today;
    ...
    const pricePax = knowledgeBase?.special_bookings?.tent_pitching?.price_php || 500;
    ...
    const total            = pricePax * (parseInt(guests) || 0) * (nights || 0);
    ...
    const checkAndAdvance = async () => {
        ...
        try {
            const url  = `${HUB_API}/special-availability?type=tent_pitching&check_in=${checkIn}&check_out=${checkOut}`;
            const resp = await fetch(url);
            const data = await resp.json();
            if (!data.available) { setError('All tent spots are taken for those dates. Please choose different dates.'); setUploading(false); return; }
    ...
    const downloadSlip = async () => {
        ...
        try {
            ...
            link.download = `amalfi-tent-${bookingRef}.png`;
                ...
                <div className={guestModal.content} onClick={e => e.stopPropagation()}>
                    ...
                    {step === 1 && (
                        <div className={guestModal.step}>
                            ...
                            <h2 className={guestModal.roomTitle}>Tent Pitching</h2>
                            <p className={guestModal.desc}>Sleep under the Liwliwa sky. Wake up to waves. PHP {pricePax.toLocaleString()}/pax per night. Bring your own tent. Subject to availability.</p>
                            <div className={guestModal.gridTwo}>
                                <div>
                                    <label className={guestModal.labelSmall}>Check-In</label>
                                    <div className={guestModal.dateWrap}>
                                        <input ref={checkInRef} type="date" className={guestModal.dateInput} min={today} value={checkIn}
                                            style={{ color: checkIn ? 'var(--text-primary)' : 'var(--text-muted)' }}
                                            onChange={e => { const v = e.target.value; setCheckIn(v); if (checkOut && checkOut <= v) setCheckOut(''); }} />
                                        <button type="button" className={guestModal.calBtn} onClick={() => checkInRef.current?.showPicker()}><CalIcon /></button>
                                    </div>
                                </div>
                                <div>
                                    <label className={guestModal.labelSmall}>Check-Out</label>
                                    <div className={guestModal.dateWrap}>
                                        <input ref={checkOutRef} type="date" className={guestModal.dateInput} min={minOut} value={checkOut}
                                            style={{ color: checkOut ? 'var(--text-primary)' : 'var(--text-muted)' }}
                                            onChange={e => setCheckOut(e.target.value)} />
                                        <button type="button" className={guestModal.calBtn} onClick={() => checkOutRef.current?.showPicker()}><CalIcon /></button>
                                    </div>
                                </div>
                                <div className={guestModal.fullSpan}>
                                    ...
                                    <p className={guestModal.hint}>
                                        {nights > 0 ? `${nights} night${nights > 1 ? 's' : ''} - PHP ${pricePax.toLocaleString()} x ${guests} pax x ${nights} nights = PHP ${total.toLocaleString()}` : 'Pricing updates based on nights and headcount.'}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className={guestModal.step}>
                            ...
                            <div className={guestModal.commitBox}>
                                ...
                                <div className={guestModal.priceRows}>
                                    <div className={`${guestModal.priceRow} ${guestModal.priceMuted}`}><span>Stay Total ({guests} pax - {nights} night{nights !== 1 ? 's' : ''})</span><span>PHP {total.toLocaleString()}</span></div>
                    ...
                    {step === 3 && (
                        <div className={guestModal.step}>
                            ...
                            <div className={uploadZoneClass(file)}>
                                <input type="file" id="tp-receipt" hidden onChange={e => { setFile(e.target.files[0]); setError(null); }} accept="image/*,application/pdf" />
                                <label htmlFor="tp-receipt" style={{ cursor: 'pointer', display: 'block' }}>
                            ...
                            {bookingRef && (
                                <div className="mt-3 p-3 bg-coastal-surfaceLow/70 rounded-xl border border-coastal-outline/50">
                                    ...
                                    <p style={{ fontSize: '0.72rem', color: '#707882', marginTop: '6px' }}>Retrying here will reuse this same tent-pitching booking.</p>
                                </div>
                            )}
                            {error && <p className={guestModal.error}>{error}</p>}
                            ...
                            <div ref={receiptRef} style={{ background: '#fef9ef', fontFamily: "'Montserrat', sans-serif", padding: '28px 24px 24px' }}>
                                <div style={{ background: 'var(--accent-green)', margin: '-28px -24px 0', padding: '22px 24px 20px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <div>
                                            ...
                                            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.58rem', fontWeight: '300', letterSpacing: '1px', margin: 0 }}>Tent Pitching Acknowledgement</p>
                                        </div>
                                ...
                                <div style={{ marginBottom: '16px' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
                                        {[['Type', 'Tent Pitching'], ['Guests', `${guests} pax`], ['Check-In (PHT)', formatDateManila(checkIn)], ['Check-Out (PHT)', formatDateManila(checkOut)], ['Nights', `${nights} night${nights !== 1 ? 's' : ''}`], ['Rate', `PHP ${pricePax.toLocaleString()}/pax/night`]].map(([label, val]) => (
                                            <div key={label}>
                                ...
                                <div style={{ marginBottom: '20px' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', color: '#707882' }}><span>Total ({guests} pax x {nights} nights x PHP {pricePax})</span><span>PHP {total.toLocaleString()}</span></div>
...
export default TentPitchingModal;
